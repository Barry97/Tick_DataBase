...
@serializable case class KafkaProducer(
	...
	props.put("kafka.producer.async.DefaultEventHandler", "enable")
	//  val producer avec son mode d'envoie
	val producer = new Producer[AnyRef, AnyRef](new ProducerConfig(props))
	
	// methode pour envoyer des messages sous forme de Byte 
	...
	// si les messages sont envoyer sous forme de string  ils sont  converti en byte kafka 
	...
	def send(message: String, partition: String = null): Unit = send(message.getBytes("UTF8"), if (partition == null) null else partition.getBytes("UTF8"))
			// deuxieme methode de conversion et d'envoyer 
	...
	//gener les données depuis un fichier sur HDFS
	def ProduceFromHDFSFile( sc: SparkContext, path: String, partition: String) {
		//val log_HDFS = sc.textFile("hdfs://sldifrdwbhn01.fr.intranet:8020" + path)
		
	  val log_HDFS = sc.textFile(path)
				//log_HDFS.collect().foreach { println }
				val send = log_HDFS.toArray().foreach {
			x =>
			val producer = new KafkaProducer(topic, brokerList, clientId, true, true, 10000, 3, -1)
			// transformation du message en Bytes 
			val message = x.getBytes("UTF8")
			println("--------------------------" + message)
			// transformation de la partition en Bytes 
			val partitions = new String(partition).getBytes("UTF8")
			//envoies du message à Kafka 
			producer.send(message, partitions)
		}
	}
	//generer les données depuis Hbase
	def ProduceFromHbase(sc: SparkContext, tableNAme: String, partition: String) {
		val Hconf = HbaseConnector.hbaseConf()
				val data1 = HbaseRead.GetResult(sc, Hconf, tableNAme).toArray()
				data1.foreach { x =>
				val producer = new KafkaProducer(topic, brokerList, clientId, true, true, 10000, 3, -1)
				producer.send(x.toString(), partition)
		}
	}
	//gener les données depuis Hive
	def ProduceFromHive(sc: SparkContext, Database: String, Table: String, partition: String) {
		val data = Hive.Read(Database, Table, sc)
				data.foreach { x =>
				val producer = new KafkaProducer(topic, brokerList, clientId, true, true, 10000, 3, -1)
				producer.send(x.toString(), partition)
		}
